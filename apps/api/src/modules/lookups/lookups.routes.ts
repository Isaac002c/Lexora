...
lookupsRouter.get("/", requireAuth, async (request, response) => {
  ...
  const data = await withTenant(auth.tenantId, async (tx) => {
    const [branches, legalAreas, users, clients, cases] = await Promise.all([
      ...
      tx.client.findMany({ where: { tenantId: auth.tenantId, status: "ACTIVE", ...(branchIds ? { primaryBranchId: { in: branchIds } } : {}), ...clientAttorneyFilter(auth) }, select: { id: true, name: true }, orderBy: { name: "asc" }, take: 500 }),
