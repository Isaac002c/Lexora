...
lookupsRouter.get("/", requireAuth, async (request, response) => {
  ...
  const data = await withTenant(auth.tenantId, async (tx) => {
    const [branches, legalAreas, users, clients, cases] = await Promise.all([
      ...
      tx.client.findMany({ where: { tenantId: auth.tenantId, deletedAt: null, status: "ACTIVE", ...(branchIds ? { primaryBranchId: { in: branchIds } } : {}), ...clientAttorneyFilter(auth) }, select: { id: true, name: true, primaryBranchId: true }, orderBy: { name: "asc" }, take: 500 }),
      tx.legalCase.findMany({ where: { tenantId: auth.tenantId, deletedAt: null, ...(branchIds ? { branchId: { in: branchIds } } : {}), ...caseAssignmentFilter(auth) }, select: { id: true, caseType: true, processNumber: true, parties: { where: { isPrimary: true }, select: { client: { select: { name: true } } } } }, orderBy: { createdAt: "desc" }, take: 500 }),
    ]);
