...
| **Cores: >7d verde, ≤7d amarelo, ≤5d vermelho** | ✅ | `lib/deadline.ts:5` + 7 testes; API confirma prazo a 4 dias = `RED` | Baixo | — | Confirmar visual na UI |
| Datas sem erro de 1 dia (fuso America/Sao_Paulo) | ✅ | `formatDate` agora usa `timeZone` explícito (UTC p/ data pura, SP p/ data+hora); TZ fixado no runtime (API + web) | Baixo | — | Resolvido (P2) — provado: data pura rende dia correto |
...
| --- | --- | --- | --- |
| R1 | ~~Erro de fuso em datas `@db.Date` (erro de 1 dia)~~ | ✅ **Mitigado** | `formatDate` corrigido (UTC p/ data pura, SP p/ data+hora) + TZ fixado no runtime; provado por teste |
...
7. ✅ Sistema mostra a cor correta do prazo. *(API: prazo a 4 dias = RED; lógica >7/≤7/≤5 com 7 testes; falta o "olho" visual na UI)*
...
- **2026-06-22 (B3 — Git)** — Criado o **commit baseline** `5ca2ab1` na `main` (172 arquivos; conferido que `.env`/segredos/`node_modules`/logs ficaram de fora; `*.tsbuildinfo` adicionado ao `.gitignore`). Repositório agora tem rede de segurança.
- **2026-06-22 (P2 — Fuso horário)** — Corrigido o "erro de um dia":
  - `apps/web/src/lib/format.ts`: `formatDate` usa `timeZone` explícito — **UTC** para data pura (`@db.Date`) e **America/Sao_Paulo** para data+hora (`Timestamptz`). Provado: valor de meia-noite UTC rende o dia correto (antes 21, depois 22).
  - `apps/api/src/server.ts` e `apps/web/src/instrumentation.ts`: fixam `TZ=America/Sao_Paulo` no runtime (janelas de prazo "hoje/próximos N", início de mês no painel, agrupamento do calendário).
  - Revalidado: `typecheck` ✅, `build` ✅; servidores reiniciados sem regressão (login + painel OK); API confirma cor de prazo (`RED` para vencimento em 4 dias).
  - **Pendente:** verificação P1 "no olho" na interface dos demais módulos (criar/editar/filtrar por perfil); revisão de entrada de datas em formulários; itens P3 de entrega.
